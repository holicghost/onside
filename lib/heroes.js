...
/** 영웅 포트레이트 URL (Blizzard CloudFront CDN) */
export function getHeroPortraitUrl(heroId) {
  const hero = ALL_HEROES.find(h => h.id === heroId);
  if (!hero) return null;
  return `https://d15f34w2p8l1cc.cloudfront.net/overwatch/${hero.cfKey}-portrait.png`;
}

const TIER_RANKS = ['브론즈', '실버', '골드', '플래티넘', '다이아몬드', '마스터', '그랜드마스터', '챔피언'];

/** 상세 티어 목록 (각 등급 5~1, 탑500) */
export const TIERS_DETAILED = [
  ...TIER_RANKS.flatMap(rank => [5, 4, 3, 2, 1].map(n => `${rank} ${n}`)),
  '탑500',
];
